...
function capitalizeNames() {
  ...
  const capItems = items.map((item) => {
    const ltr = item.name.slice(0,1)
    const capLtr = ltr.toUpperCase()
    const capItem = capLtr + item.name.slice(1)
    return {id:item.id, name:capItem, price:item.price, categoryId:item.categoryId, inventory:item.inventory}
  })
  return capItems
}
...
function calculateAllInventoryPrice() {
  ...
  const sum = items.reduce((acc, item) => {
    acc = acc + item.price * item.inventory
    return acc
  },0)
  return sum
}
